...
function buildBatchSearchPrompt(chunks: string, queries: SearchQuery[], topK: number): string {
  ...
  return `Sen deneyimli bir öğretmenisin ve eğitim kazanımları konusunda uzmansın.
...
export async function searchKazanimlar(
  dersId: number,
  queries: SearchQuery[],
  topK: number = 5
  ...
  console.log(`Searching ${queries.length} queries in batch mode`);
